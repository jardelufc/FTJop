...
public class FindUsedConstants extends JOPizerVisitor {
	...
	private void find(Method method) {
		...
		MethodGen methodGen = new MethodGen(method, clazz.getClassName(), cpool);
		...
		for (Iterator it = instructionFinder.search(cpInstr); it.hasNext();) {
		...
		CodeExceptionGen[] et = methodGen.getExceptionHandlers();
