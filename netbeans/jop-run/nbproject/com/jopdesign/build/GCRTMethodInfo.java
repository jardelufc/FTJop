...
//							.getLocals().getClone());
					...
					if (prevof != null && !frmComp(prevof, newf, theSuccessor)) {
						System.out.println("A: Gosling violation:" + prevf.toString() + newf.toString());
						System.exit(-1);
					}
...
//							.getLocals().getClone());
					...
					Frame prevof = (Frame) outFrames.put(v, newf);
					if (prevof != null && !frmComp(prevof, newf, v)) {
						System.err.println("C: Gosling violation:" + prevf.toString() + newf.toString());
						System.exit(-1);
					}
	...
	boolean frmComp(Frame prevf, Frame newf, InstructionContext ic) {
		...
		for (int i = 0; i < opprev.slotsUsed(); i++, j++) {
			...
			// peek() is per type
			if (opprev.peek(j).getSignature().equals("J") || opprev.peek(j).getSignature().equals("D")) {
				j--;
			}
		}
...
//					prevf.toString()+newf.toString()+ic.toString());
	...
	public int dumpMethodGcis(PrintWriter out) {
		...
		if (gcMethods != null && gcMethods.contains(method)) {
			if (out != null) {
				out.println("\n\t// no stackwalker info for " + mi.getCli().clazz.getClassName() + "." + mi.methodId + " because cfgReduce == true and gcMethods.contains(method)\n");
			}
		...
		if (!method.isAbstract()) {
			...
			if (out != null) {
				out.println("\n\t//stackwalker info for " + mi.getCli().clazz.getClassName() + "." + mi.methodId);
			}
